...
- Next to the editor, content pages for search engines and AI assistants (`src/seo/`): an HP and U calculator, and reference tables and guides, built with the UIPIRATE UI kit.
...
- UI kit, for the content pages only: `@salnika/uipirate` (UIPIRATE, React + vanilla-extract), vendored as a tarball in `vendor/`
...
- `src/seo/`
  - The content pages: React pages built with the UI kit, rendered to plain HTML at build time, since no major AI crawler runs JavaScript. `pages/index.ts` lists them (`CONTENT_PAGES`); each page module holds its metadata, FAQ and sections.
  - `render.tsx` is the server entry: it renders the pages, and `siteFiles.ts` the sitemap, `robots.txt` and `llms.txt`. `components/` holds the shared frame (`SeoLayout`, numbered `Section`, `DataTable`, `Faq`, `Prose`).
  - `islands/` holds the parts that run in the browser, such as the calculator: the build renders each one on its own, and its client entry (listed in `islands/entries.ts`) hydrates that HTML.
- `scripts/`
  - Small maintenance scripts, such as the SVG library manifest generator.
  - `seo/plugin.ts` publishes the content pages: at build time it runs a client build (one stylesheet and the island scripts, in `assets/seo/`, apart from the editor's chunks) and a server build of `src/seo/render.tsx`, then adds the rendered pages to the output. The dev server serves the very same files, built on the first request and again after a source file changes.
- `vendor/`
  - The UI kit's package, as the tarball its repository publishes.
- `api/`
...
- Keep new user-facing copy in the i18n layer instead of hardcoding strings in components. The content pages are the exception: they are English content, written in their page module.
- Content pages use the UI kit's components (`@salnika/uipirate`) and its tokens (`vars`) in their `.css.ts`, not the editor's theme; the editor does not use the kit. Links that look like buttons go through `ButtonLink`, since the kit's `Button` only renders `<button>`.
- Use `vanilla-extract` `.css.ts` files for styling. Do not introduce Tailwind, CSS modules, or CSS-in-JS.
- Show a page or a view that is loading with `SphereLoader` (`src/components/SphereLoader`), with a label saying what loads; `Sphere` is the drawing alone, sized and coloured by the variables of `Sphere.css.ts`. The boot splash of `index.html` draws the same sphere in plain CSS, since it shows before any script runs: change `src/lib/sphereGeometry.ts` and the splash together.
- Take icons from `lucide-react`, imported by name so only the icons used are bundled. `LucideProvider` in `App.tsx` sizes them to 16 px; pass `size` only to depart from it.
- Actions are icon-only buttons with a tooltip: use `IconButton` (`src/components/IconButton`), whose `label` is both the accessible name and the tooltip, with the `outline`, `ghost`, `primary` or `danger` variant, and `aria-pressed` for toggles. Anything else gets a tooltip from a `data-tooltip` attribute, and `iconLabelProps(label)` names an element that only shows an icon. `TooltipLayer`, mounted once in `App.tsx`, shows them on hover and keyboard focus, never on touch. Keep a text label only where it is needed: calls to action (order, buy, download), dialog buttons, menu items, buttons whose label carries a value ("Use Ø… mm"), and the toggles of the compact layout, since touch screens have no hover. An icon next to a label stays decorative (Lucide hides it from screen readers). Panel parts Lucide lacks (jack, knob, toggle, LED, insert, oval, slot) are drawn in `src/components/ElementTypeIcon/elementIcons.ts` with `createLucideIcon`, on Lucide's 24 px grid.
...
- Content pages must read whole without JavaScript: render their text and tables in the page, and keep what needs the browser in an island. Read every number from `src/lib`, as the calculator reads `src/lib/rackCalculator.ts`, so a page never quotes a size the editor no longer applies. Show the FAQ a page describes in its JSON-LD on the page itself. Content pages load no script but their island's, and never get styles or content from a script: their styles are the stylesheet the build writes, and a few lines inline in the head (`CRITICAL_STYLES` in `document.ts`) paint the dark ground and opt into the browser's cross-fade between pages before it loads, so no white shows. `index.html` opts into that cross-fade too. `src/seo/render.test.ts` checks one `h1` per page, title and description lengths, canonical URLs, internal links, JSON-LD, the scripts each page loads, and the inline styles.
...
- Add or update Vitest coverage for non-trivial business logic, especially in:
  ...
  - storage
  - export helpers
- Keep tests under the existing `src/lib/**/*.test.ts` pattern; the content pages have theirs under `src/seo/**/*.test.ts`.
...
- The UI kit is not on the public npm registry, so `package.json` points at `vendor/salnika-uipirate-<version>.tgz` and installs need no token. To update it, pack the new version in the kit's repository (`yarn pack`), replace the tarball, and run `pnpm add @salnika/uipirate@file:vendor/<tarball>`.
