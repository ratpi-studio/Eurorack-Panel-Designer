...
export function parseSerializedPanel(payload: string | SerializedPanel): SerializedPanel {
...
export function deserializePanelModel(payload: string | SerializedPanel): PanelModel {
...
function isPanelModel(value: unknown): value is PanelModel {
  ...
  const hasMounting =
    Boolean(candidate.mountingHoleConfig) && isMountingHoleConfig(candidate.mountingHoleConfig);
